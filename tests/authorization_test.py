...
from aerest.authorization import UserOwnedAuthorization
...
class Person(ndb.Model):
    ...
    def get_owner_ids(self):
        """
        This method is used by the ``UserOwnedAuthorization`` it should
        return a list of strings representing the string value of the user's id.

        :return: a list of strings representing the string value of the user's id.
        """
        try:
            return self.data['ownerIds']
        except Exception:
            return None
...
class PersonResource(NDBResource):
    ...
    # Admin only
    authorization = [AdminAuthorization, UserOwnedAuthorization]
...
def create_person(name):
    ...
    # add the user with id '1' as an owner
    # As a convention user_ids are stored as a list of strings.
    data = {'id':fid, 'name': name, 'ownerIds': ['1']}
...
class TestAdminAuthorization(BaseTestCase):

    def setUp(self):
        super(TestAdminAuthorization, self).setUp()
        ...
        self.user = create_user(role="admin", id=2)
...
class TestUserOwnedAuthorization(BaseTestCase):

    def setUp(self):
        super(TestUserOwnedAuthorization, self).setUp()
        self.register_model('Person', Person)
        self.register_model('Config', Config)
        self.register_model('User', User)
        self.register_model('UserProfile', UserProfile)
        self.user = create_user(id=1)
...
class TestUserOwnedAuthorizationUnAuthorized(BaseTestCase):

    def setUp(self):
        super(TestUserOwnedAuthorizationUnAuthorized, self).setUp()
        self.register_model('Person', Person)
        self.register_model('Config', Config)
        self.register_model('User', User)
        self.register_model('UserProfile', UserProfile)
        self.user = create_user(id=2)
